# FollowFlow — The Autonomous Enterprise Commitment Network

> **AWS Agents for Humans Hackathon** · *Professional Agents Track*  
> **Live Production App:** [followflow.duckdns.org](http://followflow.duckdns.org)  
> **GitHub Repository:** [github.com/mittai17/FollowFlow](https://github.com/mittai17/FollowFlow) (MIT Licensed)  
> **AWS Stack:** Amazon Bedrock AgentCore, AWS Strands Agents SDK v1.55.1, Amazon ECS Fargate, AWS Application Load Balancer, Amazon ECR

---

## 💡 Inspiration: The Broken Promise Tax
...
1. **Inline Strands AI Copilot:** When creating commitments, the agent parses natural language, identifies the deliverable, calculates SLA horizons, and binds the task to an evidence source (GitHub PR, S3 bucket, Slack confirmation, or Jira ticket).
2. **Autonomous Multi-Cadence Follow-Through:** Instead of spamming users, FollowFlow uses a mathematically tuned follow-through schedule. If a deadline approaches without verified proof, the agent sends gentle, context-aware nudges.
...
### Mathematical SLA & Reputation Model
...
Where:
- $w_i \in [1.0, 3.0]$ represents the business criticality weight (Low, Medium, High, Critical).
...
```
[ Clients / Users ] ──> [ DuckDNS Edge ] ──> [ AWS Application Load Balancer ]
                                                         │
               ┌─────────────────────────────────────────┴─────────────────────────────────────────┐
               ▼                                                                                   ▼
   [ Next.js 14 SSR Frontend ]                                                          [ FastAPI Backend ]
   - Tailwind CSS / Linear UI                                                           - Strands Agent Loop
   - Enterprise Sign-In Wall                                                            - Bedrock AgentCore
   - Real-time SSE Streams                                                              - SHA-256 Ledger
               │                                                                                   │
               └─────────────────────── Amazon ECS Fargate (AWS Cloud) ────────────────────────────┘
                                                         │
                                                         ▼
                                       [ Amazon Bedrock & Claude 3.5 Sonnet ]
                                       [ AWS Strands Agents SDK v1.55.1 ]
```
